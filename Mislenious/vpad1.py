...
######### underline button
underline_icon=tk.PhotoImage(file='../icons2/italic.png')
...
def change_font(main_application):
    global current_font_fa font_family: StringVar
    current_font_family= font_family.get()
    text_editor.configure(font=(current_font_family, current_font_size))

font_box.bind("<<ComboboxSelected>>", change_font)
def change_fontsize(main_application):
    global current_font_size: StringVar
...
def change_underline():
    ...
    if text_property.actual()['underline']==1:
        text_editor.configure(font=(current_font_family,current_font_size,'normal'))
italic_btn.configure(command=change_underline)
...
def changed(event=None):
    ...
    if text_editor.edit_modified():
        ...
        characters= len(text_editor.get(1.0,'end-ic'))
...
def new_file(event=None):
    ...
    text_editor.delete(1.0,tk.END)
########file
file.add_command(label='New',image=new_icon,compound=tk.LEFT, accelerator='Ctrl+N',command=new_life)
...
def exit_func(event=None):
    ...
    try:
        if text_changed:
            mbox=messagebox.askyesnocancel('warning','Do you wan to save file ?')
            if mbox is True:
                if url:
                    ...
                    content4=str(text_editor.get(1.0,tk.END))
                    url=filedialog.asksaveasfile(mode='w',defaultextension='.txt',filetypes=(('Text File', '*.txt'), ('All files', '*.*')))
                    url.write(content4)
                    url.close()
                    main_application.destroy()
...
def find_func(event=None):
    def find():
        ...
        if word:
            start_pos='1.0'
            while True:
                start_pos= text_editor.search(word,start_post,stopindex==tk.END)
                if not start_pos:
                    break
...
edit.add_command(label='Clear All',image=clear_all_icon,compound=tk.LEFT,accelerator='Ctrl+Z',command=lambda:text_editor.delete(1.0,tk.END))
